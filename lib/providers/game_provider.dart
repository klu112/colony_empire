...
class GameProvider with ChangeNotifier {
  ...
  void updateResources() {
    ...
    if (time % 10 == 0) {
      ...
      if (math.Random().nextDouble() < hatchChance && newPopulation['eggs']! > 0) {
        newPopulation['eggs'] = newPopulation['eggs']! - 1;
        newPopulation['larvae'] = newPopulation['larvae']! + 1;
      }
      
      if (math.Random().nextDouble() < hatchChance && newPopulation['larvae']! > 0) {
        newPopulation['larvae'] = newPopulation['larvae']! - 1;
    ...
    final newResources = resources.copyWith(
      food: (resources.food + foodChange - (newPopulation['workers']! * 0.05))
          .clamp(0.0, 100.0),
      buildingMaterials: (resources.buildingMaterials + materialChange)
          .clamp(0.0, 100.0),
      water: (resources.water + waterChange - 0.1)
          .clamp(0.0, 100.0),
  ...
  void _initializeAnts() {
    ...
    for (int i = 0; i < resources.population['workers']!; i++) {
      final targetChamber = chambers[math.Random().nextInt(chambers.length)];
      final taskList = ['foraging', 'building', 'caregiving', 'defense', 'exploration'];
    ...
    final events = [
      {
        ...
        'effect': () {
          if (taskAllocation.defense < 10) {
            ...
            final newPopulation = Map.from(resources.population);
            ...
            setNotification('Deine Soldaten haben das Nest erfolgreich verteidigt!');
          }
        }
      },
      {
